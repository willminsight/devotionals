...
function iflm_devo_meta_callback( $post ) {
	...
	<?php
} //end of function: iflm_devo_meta_callback
...
function enqueue_iflm_devo_styles() {
	if ( is_singular('iflm_devotional') ) {
		...
		wp_enqueue_style( 'devotionals', plugin_dir_url( __FILE__ ) . 'css/devotionals.css' );
	}
...
function iflm_devo_content_feed( $content ) {
	...
	if ( $iflm_devo_post_type == 'iflm_devotional') {
		...
		*/
		...
		/* 
		 * END: Add biblegateway.com links to scripture passages in RSS Feed 
		*/
...
add_filter( 'pre_get_posts', 'iflm_devo_add_custom_types' );
?>
